...
def main():
	...
	celeba_test_images = utilities.load_images(100, celeba_test_img_file_names)
...
def train_cnn(utility_obj, training_data, training_label,celeba_test_images,celeba_test_labels):
	W_conv1 = utility_obj.weight_variable([5, 5, 1, 60])
	b_conv1 = utility_obj.bias_variable([60])
	...
	W_conv2 = utility_obj.weight_variable([5, 5, 60, 120])
	b_conv2 = utility_obj.bias_variable([120])

	h_conv2 = tf.nn.relu(utility_obj.conv2d(h_pool1, W_conv2) + b_conv2)
	h_pool2 = utility_obj.max_pool_2x2(h_conv2)

	W_fc1 = utility_obj.weight_variable([14 * 14 * 120, 1024])
	b_fc1 = utility_obj.bias_variable([1024])

	h_pool2_flat = tf.reshape(h_pool2, [-1, 14 * 14 * 120])
